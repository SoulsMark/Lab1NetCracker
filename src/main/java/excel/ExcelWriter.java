...
public class ExcelWriter {
    ...
    public static void toExcelWriterXLSX(String fileName, int arrLength, long time, Set<Class<?>> annotated) {
        try {
            String path = fileName + EXCEL_FILE_EXTENSION;
     ...
     */
    public static void createCharts(int exNumber) throws IOException {
        FillArrays runner = new FillArrays();
        Method[] methods = runner.getClass().getMethods();
        for (Method m : methods) {
            Filler annos = m.getAnnotation(Filler.class);
            if (annos != null) {
                createChart(m.getName(), exNumber);
            }
        }
    }
    ...
    private static void createChart(String name, int exNumber) throws IOException {
        File file = new File(name + EXCEL_FILE_EXTENSION);
        FileInputStream inp = new FileInputStream(file);
        ...
        FileOutputStream fileOut = new FileOutputStream(name + EXCEL_FILE_EXTENSION);
     ...
     */
    private static void writeData(XSSFSheet sheet, int methods, int arrLength, long time) {
     ...
     */
    private static void writeHeading(XSSFSheet sheet, int methods, Set<Class<?>> annotated) {
     ...
     * @return Set as array.
     */
    private static String[] setElements(Set<Class<?>> set) {
     ...
     */
    private static void writeAndClose(Workbook workbook, OutputStream fileOut) {
        try {
